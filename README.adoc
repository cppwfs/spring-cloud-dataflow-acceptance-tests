...
Tests that check the base Spring Cloud Data Flow on how it handles a streams
...
container as well as the Spring Cloud Data Flow Server.
...
`./startAcceptanceTests.sh -s -d`.  This will skip the download of the SCDF
server jar and not start the rabbit nor SCDF-Server-Local instances,
...
up an instance of Spring Cloud Data Flow Server Local, execute the following:
`./startAcceptanceTests.sh -sb`.

==== Get Help
Execute `./startAcceptanceTests.sh` and the following will be displayed:
